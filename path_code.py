...
def main(image_filename):
    ...
    for (x, y, window) in traversal_window(image, stepSize=60, windowSize=(winW, winH)):
        ...
        if (any(i <= 240 for i in average_color)):  # Check if grids are colored
            ...
            full_grids.append(tuple(index))  # These grids are termed as occupied_grids
        ...
        if (any(i <= 20 for i in average_color)):  # Check if grids are black in color
            ...
            barrier.append(tuple(index))  # add to obstacles list
    ...
    for startimage in list_colored_grids:
        ...
        for grid in [n for n in list_colored_grids if n != startimage]:
            ...
            if s > 0.9:
                ...
                result = shortest_path_algorithm(maze, (startimage[0] - 1, startimage[1] - 1),(grid[0] - 1, grid[1] - 1))
                #print(result)
                list2 = []
    ...
    print("Dictionary Keys of expected_path:")
